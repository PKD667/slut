...
pub static C: LazyLock<Scalar<f64,Velocity>> = LazyLock::new(|| Scalar::default([ 299_792_458.0 ]));
// Gravitational constant in m³/(kg·s²)
pub static G: LazyLock<Scalar<f64,dim_mul!(
    Length,
    (dim_mul!(
        Mass,
        (dim_inv!(Time))
    ))
)>> = LazyLock::new(|| Scalar::default([ 6.67430e-11]));
// gravity on Earth's surface in m/s²
pub static G_EARTH: LazyLock<Scalar<f64,Acceleration>> = LazyLock::new(|| Scalar::default([ 9.80665 ]));
// planck constant in J·s
pub static H: LazyLock<Scalar<f64,dim_mul!(Energy,Frequency)>> = LazyLock::new(|| Scalar::default([ 6.62607015e-34]));
// Boltzmann constant in J/K
pub static K: LazyLock<Scalar<f64,Entropy>> = LazyLock::new(|| Scalar::default([ 1.380649e-23]));